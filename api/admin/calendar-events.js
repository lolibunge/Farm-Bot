...
async function checkExistingTables(tables) {
...
function buildCalendarQuery(existingTables) {
  const parts = [];
  ...
  parts.push(`
    ...
    WHERE
      ...
      AND (r.rain_mm > 0 OR COALESCE(NULLIF(TRIM(r.notes), ''), '') <> '')
  `);
  ...
  if (existingTables.has('feed_events') && existingTables.has('feed_items')) {
    parts.push(`
      SELECT
  ...
  if (existingTables.has('deworming_events')) {
    parts.push(`
      SELECT
  ...
  if (existingTables.has('farrier_events')) {
    parts.push(`
      SELECT
  ...
  if (existingTables.has('horse_health_events')) {
    parts.push(`
      SELECT
  ...
  if (existingTables.has('stock_events') && existingTables.has('feed_items')) {
    parts.push(`
      SELECT
  ...
  if (existingTables.has('treatment_plans')) {
    parts.push(`
      SELECT
  ...
  if (existingTables.has('treatment_logs') && existingTables.has('treatment_plans')) {
    parts.push(`
      SELECT
  ...
  parts.push(`
    SELECT
      CONCAT('visit-', fv.id) AS event_key,
      fv.event_date::timestamp AS event_at,
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const calendarEventsResult = await pool.query(
      buildCalendarQuery(existingTables),
