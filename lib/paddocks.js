...
async function findHorseGroupByName(name, options = {}) {
  ...
  let sql = `
    ...
    LEFT JOIN LATERAL (
      SELECT
        MIN(hgh.started_at) AS current_started_at
      FROM horse_group_membership_history hgh
      WHERE hgh.group_id = g.id
        AND hgh.ended_at IS NULL
    ) current_membership ON TRUE
    LEFT JOIN LATERAL (
      SELECT
        STRING_AGG(DISTINCT p.name, ', ' ORDER BY p.name) AS current_paddock_names,
        ARRAY_AGG(DISTINCT p.id ORDER BY p.id) AS current_paddock_ids,
        CASE
          WHEN COUNT(DISTINCT ge.entered_at) = 1 THEN MIN(ge.entered_at)
          ELSE NULL
    ...
    for (const memberRow of memberResult.rows) {
      ...
      if (shouldShiftLatestExit) {
        ...
        await client.query(
          ...
          WHERE id = $2
          `,
          [targetEnteredAt, latestClosedEvent.id]
        );
      }
  ...
  const result = await pool.query(
    ...
    SELECT
      ...
      CASE
        ...
        WHEN manual_rest.manual_rest_started_on IS NULL AND latest_exit.last_exited_at IS NULL AND p.active = TRUE
          THEN p.id
        WHEN manual_rest.manual_rest_started_on IS NULL THEN latest_exit.effective_rest_paddock_id
        WHEN latest_exit.last_exited_at IS NULL THEN manual_rest.effective_rest_paddock_id
        WHEN manual_rest.manual_rest_started_on >= latest_exit.last_exited_at
          THEN manual_rest.effective_rest_paddock_id
        ELSE latest_exit.effective_rest_paddock_id
      END AS effective_rest_paddock_id,
      CASE
        WHEN COALESCE(current_occupancy.horse_count, 0) > 0 THEN NULL
        WHEN manual_rest.manual_rest_started_on IS NULL AND latest_exit.last_exited_at IS NULL AND p.active = TRUE
          THEN p.name
        WHEN manual_rest.manual_rest_started_on IS NULL THEN latest_exit.effective_rest_paddock_name
        WHEN latest_exit.last_exited_at IS NULL THEN manual_rest.effective_rest_paddock_name
        WHEN manual_rest.manual_rest_started_on >= latest_exit.last_exited_at
          THEN manual_rest.effective_rest_paddock_name
        ELSE latest_exit.effective_rest_paddock_name
      END AS effective_rest_paddock_name,
      CASE
        WHEN COALESCE(current_occupancy.horse_count, 0) > 0 THEN NULL
        WHEN manual_rest.manual_rest_started_on IS NULL AND latest_exit.last_exited_at IS NULL AND p.active = TRUE
          THEN '${DEFAULT_PADDOCK_REST_BASELINE_DATE}'::date
        WHEN manual_rest.manual_rest_started_on IS NULL THEN latest_exit.last_exited_at
        WHEN latest_exit.last_exited_at IS NULL THEN manual_rest.manual_rest_started_on
        WHEN manual_rest.manual_rest_started_on >= latest_exit.last_exited_at
          THEN manual_rest.manual_rest_started_on
        ELSE latest_exit.last_exited_at
      END AS effective_rest_started_on,
      CASE
        WHEN COALESCE(current_occupancy.horse_count, 0) > 0 THEN FALSE
        WHEN manual_rest.manual_rest_started_on IS NULL AND latest_exit.last_exited_at IS NULL AND p.active = TRUE
          THEN TRUE
        WHEN manual_rest.manual_rest_started_on IS NULL THEN FALSE
        WHEN latest_exit.last_exited_at IS NULL THEN manual_rest.manual_rest_is_estimated
        WHEN manual_rest.manual_rest_started_on >= latest_exit.last_exited_at
          THEN manual_rest.manual_rest_is_estimated
        ELSE FALSE
      ...
      CASE
        ...
        WHEN manual_rest.manual_rest_started_on IS NULL AND latest_exit.last_exited_at IS NULL AND p.active = TRUE
          THEN 'baseline'
        WHEN manual_rest.manual_rest_started_on IS NULL AND latest_exit.last_exited_at IS NULL THEN NULL
        WHEN manual_rest.manual_rest_started_on IS NULL THEN 'history'
        WHEN latest_exit.last_exited_at IS NULL THEN 'manual'
        WHEN manual_rest.manual_rest_started_on >= latest_exit.last_exited_at THEN 'manual'
        ELSE 'history'
      END AS rest_source,
      CASE
        WHEN COALESCE(current_occupancy.horse_count, 0) > 0 THEN NULL
        WHEN manual_rest.manual_rest_started_on IS NULL AND latest_exit.last_exited_at IS NULL AND p.active = TRUE
          THEN GREATEST(0, CURRENT_DATE - '${DEFAULT_PADDOCK_REST_BASELINE_DATE}'::date)::int
        WHEN manual_rest.manual_rest_started_on IS NULL AND latest_exit.last_exited_at IS NULL THEN NULL
        WHEN manual_rest.manual_rest_started_on IS NULL
          THEN GREATEST(0, CURRENT_DATE - latest_exit.last_exited_at)::int
        WHEN latest_exit.last_exited_at IS NULL
