...
const PADDOCK_SHARED_GRAZING_COLUMNS = [
  { name: 'entered_at', definition: 'DATE' },
  { name: 'exited_at', definition: 'DATE' },
  { name: 'notes', definition: 'TEXT' },
  { name: 'source', definition: "TEXT DEFAULT 'manual'" },
...
function normalizePaddockStatusRow(row) {
  ...
  return {
    ...
    has_shared_access: Boolean(row.has_shared_access),
    grazing_days: row.grazing_days == null ? null : Number(row.grazing_days),
...
async function ensurePaddockTables() {
  ...
  ensurePromise = (async () => {
    ...
    await pool.query(`
      CREATE TABLE IF NOT EXISTS paddock_shared_grazing (
        id BIGSERIAL PRIMARY KEY,
        paddock_id BIGINT NOT NULL REFERENCES paddocks(id) ON DELETE CASCADE,
        group_id BIGINT NOT NULL REFERENCES horse_groups(id) ON DELETE CASCADE,
        entered_at DATE NOT NULL,
    ...
    await ensureTableColumns(pool, 'paddock_shared_grazing', PADDOCK_SHARED_GRAZING_COLUMNS);

    await pool.query(`
      CREATE INDEX IF NOT EXISTS paddock_shared_grazing_paddock_idx
      ON paddock_shared_grazing (paddock_id, entered_at DESC, id DESC)
    `);

    await pool.query(`
      CREATE INDEX IF NOT EXISTS paddock_shared_grazing_group_idx
      ON paddock_shared_grazing (group_id, entered_at DESC, id DESC)
    `);

    await pool.query(`
      CREATE UNIQUE INDEX IF NOT EXISTS paddock_shared_grazing_open_paddock_unique_idx
      ON paddock_shared_grazing (paddock_id)
      WHERE exited_at IS NULL
    `);
...
// Sets a new "ready to graze" date directly on whichever paddock_work_event
// is currently governing a paddock's rest period (e.g. you estimated 60
// days but it didn't rain, so you push the date out further). This edits
// the event in place rather than creating a new one, and keeps
// ready_after_days consistent with the new date (recomputed from the
// event's own event_date), so the "why" (original estimate) stays visible
// in history even though the effective date changed.
async function extendPaddockReadyDate({
  paddockId,
  readyToGrazeOn,
  notes = null,
  telegramUserId = null,
  ...
  const normalizedPaddockId = Number.parseInt(paddockId, 10);
  if (!Number.isFinite(normalizedPaddockId) || normalizedPaddockId <= 0) {
    throw buildServiceError('Paddock is invalid.', 400);
  }

  const normalizedReadyDate = toIsoDateString(readyToGrazeOn);
  if (!normalizedReadyDate) {
    throw buildServiceError('New ready-to-graze date is invalid.', 400);
  }
  ...
  try {
    ...
    const activeEvent = await getEffectivePaddockWaitEvent(client, normalizedPaddockId, currentDateString());

    if (!activeEvent) {
      throw buildServiceError('This paddock has no active rest period to extend.', 404);
    }

    if (normalizedReadyDate < activeEvent.event_date) {
      throw buildServiceError('The new ready date has to be on or after the work date.', 400);
    }

    const newReadyAfterDays = Math.round(
      (new Date(`${normalizedReadyDate}T00:00:00Z`) - new Date(`${activeEvent.event_date}T00:00:00Z`)) /
        (24 * 60 * 60 * 1000)
    );

    const mergedNotes = notes
      ? [activeEvent.notes, notes].filter(Boolean).join(' | ')
      : activeEvent.notes;

    const updateResult = await client.query(
      `
      UPDATE paddock_work_events
      SET ready_after_days = $1,
          ready_to_graze_on = $2,
          notes = $3,
      ...
      RETURNING
        ...
        updated_at
      `,
      [newReadyAfterDays, normalizedReadyDate, mergedNotes, telegramUserId, activeEvent.id]
    );
    ...
    return {
      ...
      paddock_work_event: normalizePaddockWorkRow({
        ...
        days_until_ready: Math.max(
          0,
          Math.round(
            (new Date(`${normalizedReadyDate}T00:00:00Z`) - new Date(`${currentDateString()}T00:00:00Z`)) /
              (24 * 60 * 60 * 1000)
          )
        ),
      }),
...
// Applies a desired set of "shared" paddocks (extra grazing/water access alongside a
// group's primary paddock) for a horse group, diffing against what's currently open in
// paddock_shared_grazing. Unlike grazing_events, this is tracked at the group level, not
// per-horse, so it doesn't touch the one-open-paddock-per-horse invariant.
async function applySharedPaddocksForGroup(
  client,
  {
    groupId,
    paddockIds = [],
  ...
  const normalizedEnteredAt = toIsoDateString(enteredAt) || currentDateString();
  const normalizedIds = [
    ...new Set(
      (paddockIds || [])
        .map((value) => Number(value))
  ...
  const openResult = await client.query(
    `
    SELECT psg.id, psg.paddock_id, p.name AS paddock_name
    FROM paddock_shared_grazing psg
    JOIN paddocks p ON p.id = psg.paddock_id
    WHERE psg.group_id = $1
      AND psg.exited_at IS NULL
    FOR UPDATE OF psg
    `,
    [groupId]
  );

  const openByPaddockId = new Map(openResult.rows.map((row) => [Number(row.paddock_id), row]));
  const idsToClose = openResult.rows
    .map((row) => Number(row.paddock_id))
    .filter((id) => !normalizedIds.includes(id));
  const idsToOpen = normalizedIds.filter((id) => !openByPaddockId.has(id));

  if (idsToClose.length > 0) {
    const eventIdsToClose = openResult.rows
      .filter((row) => idsToClose.includes(Number(row.paddock_id)))
      .map((row) => Number(row.id));
    await client.query(
      `
      UPDATE paddock_shared_grazing
      SET exited_at = $1, updated_at = NOW()
      WHERE id = ANY($2::bigint[])
      `,
      [normalizedEnteredAt, eventIdsToClose]
    );
  }

  if (idsToOpen.length > 0) {
    const paddockCheckResult = await client.query(
      `SELECT id, name, active FROM paddocks WHERE id = ANY($1::bigint[])`,
      [idsToOpen]
    );
    const foundIds = new Set(paddockCheckResult.rows.map((row) => Number(row.id)));
    const missingIds = idsToOpen.filter((id) => !foundIds.has(id));
    if (missingIds.length > 0) {
      throw buildServiceError(`Paddock(s) not found: ${missingIds.join(', ')}`, 404);
    }

    const inactiveRows = paddockCheckResult.rows.filter((row) => !row.active);
    if (inactiveRows.length > 0) {
      throw buildServiceError(
        `Paddock is inactive: ${inactiveRows.map((row) => row.name).join(', ')}`,
        409
      );
    }

    // Hand off from whichever other group currently has shared access to these paddocks.
    await client.query(
      `
      UPDATE paddock_shared_grazing
      SET exited_at = $1, updated_at = NOW()
      WHERE paddock_id = ANY($2::bigint[])
        AND exited_at IS NULL
        AND group_id <> $3
      `,
      [normalizedEnteredAt, idsToOpen, groupId]
    );

    for (const paddockId of idsToOpen) {
      await assertPaddockReadyForEntry(client, paddockId, normalizedEnteredAt);
    }

    await client.query(
      `
      INSERT INTO paddock_shared_grazing (
        paddock_id, group_id, movement_batch_id, entered_at, notes, source, telegram_user_id
      )
      SELECT paddock_id, $2, $3, $4, $5, $6, $7
      FROM UNNEST($1::bigint[]) AS paddock_id
      `,
      [idsToOpen, groupId, movementBatchId, normalizedEnteredAt, notes, source, telegramUserId]
    );
  }

  const paddockNameResult = normalizedIds.length
    ? await client.query(`SELECT id, name FROM paddocks WHERE id = ANY($1::bigint[])`, [normalizedIds])
    : { rows: [] };
  const nameById = new Map(paddockNameResult.rows.map((row) => [Number(row.id), row.name]));

  return {
    current_shared_paddocks: normalizedIds
      .map((id) => ({ id, name: nameById.get(id) || null }))
      .sort((left, right) => String(left.name).localeCompare(String(right.name))),
    added_paddock_ids: idsToOpen,
    removed_paddock_ids: idsToClose,
  };
}

// Sets the full current set of "shared" paddocks (extra grazing/water access) for a group
// that is already grazing somewhere, e.g. a group in Potrero 4.1-4.3 that also eats in
// Potrero 3. Paddocks not included in paddockIds are released (exited_at is stamped).
async function setGroupSharedPaddocks({
  groupId,
  paddockIds = [],
  effectiveDate,
  notes = null,
  ...
  try {
    ...
    const openPrimaryResult = await client.query(
      `
      SELECT ge.paddock_id, p.name AS paddock_name
    ...
    if (openPrimaryResult.rows.length === 0) {
      throw buildServiceError(
        `${groupRow.name} is not currently grazing anywhere. Move the group in first.`,
        409
      );
    }

    const normalizedEnteredAt = toIsoDateString(effectiveDate) || currentDateString();

    const movementBatch = await createMovementBatch(client, {
      movementType: 'group_shared_paddocks_update',
      effectiveDate: normalizedEnteredAt,
      sourceGroupId: groupId,
      targetGroupId: groupId,
      targetLocationId: Number(openPrimaryResult.rows[0].paddock_id) || null,
      reason: 'Group shared paddock access updated',
      notes: notes || null,
      performedBy: telegramUserId || null,
    });
    const movementBatchId = Number(movementBatch?.id || 0) || null;

    const sharedResult = await applySharedPaddocksForGroup(client, {
      groupId,
      paddockIds,
    ...
    return {
      ...
      primary_paddock: {
        id: Number(openPrimaryResult.rows[0].paddock_id),
        name: openPrimaryResult.rows[0].paddock_name,
      },
      effective_date: normalizedEnteredAt,
      ...sharedResult,
    };
  } catch (error) {
    await client.query('ROLLBACK');
...
async function moveHorseGroupIntoPaddock({
  ...
  try {
    ...
    if (invalidDateRows.length > 0) {
      ...
      // force = true: the caller has confirmed they want to correct these horses'
      // current-paddock entry date to match the move date before transferring them.
      // Only allowed when every conflicting horse is currently in the same source
      // paddock (mixed-paddock corrections are ambiguous from a single move screen),
      // and only when it wouldn't overlap that horse's own earlier grazing history.
      const invalidSourcePaddockIds = [
        ...new Set(invalidDateRows.map((row) => Number(row.paddock_id))),
      ];
      if (invalidSourcePaddockIds.length > 1) {
        throw buildServiceError(
          `Can't auto-correct the entry date because the affected horses are split across more than one paddock: ${invalidDateRows
            .map((row) => `${row.horse_name} (${row.paddock_name})`)
            .join(', ')}. Correct each paddock separately first.`,
          400
        );
      }

      const invalidHorseIds = invalidDateRows.map((row) => Number(row.horse_id));
      const priorClosedResult = await client.query(
        `
        SELECT
          horse_rows.horse_id,
          ge.id,
      ...
      const priorClosedByHorseId = new Map(
        priorClosedResult.rows
          .filter((row) => row.id != null)
          .map((row) => [Number(row.horse_id), row])
      );

      const uncorrectableRows = invalidDateRows.filter((row) => {
        const prior = priorClosedByHorseId.get(Number(row.horse_id));
        if (!prior) {
          return false;
        }
        const priorEnteredAt = toIsoDateString(prior.entered_at);
        return priorEnteredAt && targetEnteredAt < priorEnteredAt;
      });
      if (uncorrectableRows.length > 0) {
        throw buildServiceError(
          `Can't auto-correct the entry date for: ${uncorrectableRows
            .map((row) => {
              const prior = priorClosedByHorseId.get(Number(row.horse_id));
              return `${row.horse_name} (previous stay started ${toIsoDateString(prior.entered_at)})`;
            })
            .join(', ')}. Choose a later date or correct their earlier history first.`,
          400
        );
      }

      for (const row of invalidDateRows) {
        const prior = priorClosedByHorseId.get(Number(row.horse_id));
        if (prior && toIsoDateString(prior.exited_at) === toIsoDateString(row.entered_at)) {
          await client.query(
            `
            UPDATE grazing_events
            SET exited_at = $1,
    ...
    const additionalPaddockIdsFiltered = (additionalPaddockIds || []).filter(
      (id) => Number(id) !== Number(paddockId)
    );
    const sharedResult =
      additionalPaddockIdsFiltered.length > 0
        ? await applySharedPaddocksForGroup(client, {
            groupId,
            paddockIds: additionalPaddockIdsFiltered,
            enteredAt: targetEnteredAt,
            movementBatchId,
            notes: entryNotes,
            source: source || 'group_manual',
            telegramUserId,
          })
        : { current_shared_paddocks: [], added_paddock_ids: [], removed_paddock_ids: [] };
    ...
    const responsePayload = {
      shared_paddocks: sharedResult.current_shared_paddocks,
      group: normalizeHorseGroupRow({
        ...groupRow,
        member_count: memberResult.rows.length,
        member_ids: memberResult.rows.map((row) => row.id),
...
async function moveHorseGroupOutOfPaddock({
  ...
  try {
    ...
    const releasedSharedResult = await client.query(
      `
      UPDATE paddock_shared_grazing
      SET exited_at = $1,
          movement_batch_id = $3,
      ...
      `,
      [exitedAt, groupId, movementBatchId]
    );
    ...
    return {
      ...
      released_shared_paddock_ids: releasedSharedResult.rows.map((row) => Number(row.paddock_id)),
      paddock: {
...
async function listPaddockStatus(options = {}) {
  ...
  const result = await pool.query(
    ...
    SELECT
      ...
      COALESCE(current_occupancy.has_shared_access, FALSE) AS has_shared_access,
    ...
    LEFT JOIN paddocks parent ON parent.id = p.parent_paddock_id
    LEFT JOIN LATERAL (
      WITH direct_horses AS (
        SELECT
          h.id AS horse_id,
          h.name AS horse_name,
          hgm.group_id AS group_id,
          hg.name AS group_name,
          ge.entered_at AS entered_at
        FROM grazing_events ge
        JOIN horses h ON h.id = ge.horse_id
      ...
      shared_horses AS (
        SELECT
          h.id AS horse_id,
          h.name AS horse_name,
          psg.group_id AS group_id,
          sg.name AS group_name,
          psg.entered_at AS entered_at
        FROM paddock_shared_grazing psg
        JOIN horse_groups sg ON sg.id = psg.group_id
        JOIN horse_group_memberships hgm ON hgm.group_id = psg.group_id
        JOIN horses h ON h.id = hgm.horse_id
        WHERE psg.paddock_id = p.id
          AND psg.exited_at IS NULL
      ),
      combined_horses AS (
        SELECT * FROM direct_horses
        UNION ALL
        SELECT sh.*
        FROM shared_horses sh
        WHERE NOT EXISTS (
          SELECT 1 FROM direct_horses dh WHERE dh.horse_id = sh.horse_id
        )
      )
      SELECT
        STRING_AGG(DISTINCT horse_name, ', ' ORDER BY horse_name) AS horse_names,
        STRING_AGG(DISTINCT group_name, ', ' ORDER BY group_name) AS group_names,
        COUNT(DISTINCT horse_id)::int AS horse_count,
        COUNT(DISTINCT horse_id) FILTER (WHERE group_id IS NULL)::int AS ungrouped_horse_count,
        MIN(entered_at) AS occupied_since,
        BOOL_OR(entered_at IS NOT NULL AND horse_id IN (SELECT horse_id FROM shared_horses)) AS has_shared_access
      FROM combined_horses
    ...
    LEFT JOIN LATERAL (
      ...
      SELECT last_exited_at, effective_rest_paddock_id, effective_rest_paddock_name
      FROM (
        SELECT
          ge.exited_at AS last_exited_at,
          paddock_chain.id AS effective_rest_paddock_id,
          paddock_chain.name AS effective_rest_paddock_name,
          paddock_chain.depth AS depth,
          ge.id AS tie_breaker_id
        FROM paddock_chain
        JOIN grazing_events ge ON ge.paddock_id = paddock_chain.id
        WHERE ge.exited_at IS NOT NULL
        UNION ALL
        SELECT
          psg.exited_at AS last_exited_at,
          paddock_chain.id AS effective_rest_paddock_id,
          paddock_chain.name AS effective_rest_paddock_name,
          paddock_chain.depth AS depth,
          psg.id AS tie_breaker_id
        FROM paddock_chain
        JOIN paddock_shared_grazing psg ON psg.paddock_id = paddock_chain.id
        WHERE psg.exited_at IS NOT NULL
      ) combined_exits
      ORDER BY last_exited_at DESC, depth ASC, tie_breaker_id DESC
      LIMIT 1
    ) latest_exit ON TRUE
    LEFT JOIN LATERAL (
      WITH RECURSIVE paddock_chain AS (
...
async function deletePaddock({ paddockId } = {}) {
  await ensurePaddockTables();

  const normalizedPaddockId =
    paddockId == null || paddockId === '' ? null : Number.parseInt(paddockId, 10);
  if (!Number.isFinite(normalizedPaddockId) || normalizedPaddockId <= 0) {
    throw buildServiceError('El id del potrero no es válido.', 400);
  }
  ...
  try {
    ...
    if (paddockResult.rows.length === 0) {
      throw buildServiceError('No encontramos ese potrero.', 404);
    }

    const paddockRow = paddockResult.rows[0];

    const childResult = await client.query(
      `
      SELECT id, name
      FROM paddocks
      WHERE parent_paddock_id = $1
      ORDER BY name ASC
      `,
      [normalizedPaddockId]
    );
    ...
    if (childResult.rows.length > 0) {
      const childNames = childResult.rows.map((row) => row.name).slice(0, 5).join(', ');
      throw buildServiceError(
        `No podés eliminar "${paddockRow.name}" porque tiene sub-potreros (${childNames}). Reasigná o eliminá esos primero.`,
        409
      );
    }
    ...
    const openCount = Number(occupancyResult.rows[0]?.open_count || 0);
    if (openCount > 0) {
      throw buildServiceError(
        `No podés eliminar "${paddockRow.name}" porque todavía tiene ${openCount} caballo(s) adentro. Movelos antes de borrar el potrero.`,
        409
      );
    }
    ...
    const workEventsResult = await client.query(
      'DELETE FROM paddock_work_events WHERE paddock_id = $1',
      [normalizedPaddockId]
    );
    if (workEventsResult.rowCount > 0) {
      deletedRelations.paddock_work_events = workEventsResult.rowCount;
    }

    const grazingHistoryResult = await client.query(
      'DELETE FROM grazing_events WHERE paddock_id = $1',
      [normalizedPaddockId]
    );
    if (grazingHistoryResult.rowCount > 0) {
      deletedRelations.grazing_events = grazingHistoryResult.rowCount;
    }

    await client.query(
      'UPDATE movement_batches SET source_location_id = NULL WHERE source_location_id = $1',
      [normalizedPaddockId]
    );
    await client.query(
      'UPDATE movement_batches SET target_location_id = NULL WHERE target_location_id = $1',
      [normalizedPaddockId]
    );

    await client.query('DELETE FROM paddocks WHERE id = $1', [normalizedPaddockId]);
    ...
    return {
      ok: true,
      paddock: { id: Number(paddockRow.id), name: paddockRow.name },
