...
async function getOwnersDashboard() {
  ...
  const existing = await tablesExist(['feed_events', 'feed_items', 'farrier_events', 'deworming_events']);
  ...
  const serviceQuery = (existing.has('farrier_events') || existing.has('deworming_events'))
    ? pool.query(`
        SELECT
          h.owner_id,
          ${existing.has('farrier_events') ? `COUNT(fa.id) FILTER (WHERE fa.event_date BETWEEN $1 AND $2)::int` : '0'} AS farrier_count,
          ${existing.has('deworming_events') ? `COUNT(de.id) FILTER (WHERE de.event_date BETWEEN $1 AND $2)::int` : '0'} AS deworm_count
        FROM horses h
        ${existing.has('farrier_events') ? 'LEFT JOIN farrier_events fa ON fa.horse_id = h.id' : ''}
        ${existing.has('deworming_events') ? 'LEFT JOIN deworming_events de ON de.horse_id = h.id' : ''}
        WHERE h.owner_id IS NOT NULL
        GROUP BY h.owner_id
      `, [start, end])
    : Promise.resolve({ rows: [] });
