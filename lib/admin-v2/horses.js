...
const { ensureOwnersSchema } = require('../owners');
const {
  ensurePaddockTables,
  listPaddockStatus,
  listHorseGroups,
...
const ALLOWED_SEX_VALUES = new Set(['', 'mare', 'stallion', 'gelding', 'filly', 'colt', 'unknown']);
...
function normalizeHorseProfileRow(row, { trainingModuleEnabled }) {
  ...
  return {
    ...
    feed_enabled: Boolean(row.feed_enabled),
    care: {
      deworming: {
...
function validateHorseInput(input, { trainingModuleEnabled, requireHorseName = true } = {}) {
  ...
  const ownerId = input.ownerId ? (parseInt(String(input.ownerId), 10) || null) : null;
  const feedEnabled = input.feedEnabled === true || input.feedEnabled === 'true';
