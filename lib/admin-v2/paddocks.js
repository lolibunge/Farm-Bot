...
async function getAdminV2PaddocksDashboard() {
  ...
  return {
    ...
    paddocks_dashboard: {
      ...
      paddock_map: farmSettings.paddock_map || null,
      tabs: [
        {
          key: 'cards',
...
async function deleteAdminV2Paddock(paddockId) {
  const normalizedPaddockId = parsePositiveInt(paddockId);
  if (!normalizedPaddockId) {
    throw buildServiceError('paddockId is required', 400);
  }

  const data = await deletePaddock({ paddockId: normalizedPaddockId });

  return {
    ok: true,
    paddock: data.paddock,
    deleted_relations: data.deleted_relations,
    deleted_related_row_count: data.deleted_related_row_count,
  };
}
