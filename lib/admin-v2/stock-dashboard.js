...
async function buildFeedAccountingModel(client, feedInventoryModel) {
  ...
  for (const row of feedPlanRowsResult.rows) {
    ...
    if (unitCost != null) {
      ...
      horseRow.estimated_daily_cost += toCurrencyNumber(quantity * unitCost);
    } else {
  ...
  const averageMonthlyPerHorse =
    horsesWithEstimatedCost > 0 ? toCurrencyNumber(estimatedMonthlyTotal / horsesWithEstimatedCost) : 0;
  ...
  return {
    ...
    summary_cards: [
      ...
      },
      {
        ...
        label: 'Alimento mensual estimado',
        value: estimatedMonthlyTotal,
        value_format: 'currency',
        detail:
          feedPlanRowsResult.rows.length > 0
            ? 'Proyeccion diaria convertida a 30 dias.'
            : 'Carga planes de comida para ver costos mensuales por caballo.',
        tone: estimatedMonthlyTotal > 0 ? 'blue' : 'orange',
        icon: 'calendar',
      },
      {
