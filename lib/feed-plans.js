...
const { toIsoDateString, todayDateString } = require('./date-helpers');
...
function accumulateFeedItemQuantity(map, feedItemId, quantity) {
  const normalizedFeedItemId = Number(feedItemId);
  const normalizedQuantity = Number(quantity);
  if (!Number.isFinite(normalizedFeedItemId) || normalizedFeedItemId <= 0) {
    return;
  }
  ...
  map.set(
    normalizedFeedItemId,
    Number(map.get(normalizedFeedItemId) || 0) + normalizedQuantity
  );
}

function recordStockChange(stockChangeMap, row) {
  const feedItemId = Number(row?.feed_item_id);
  if (!Number.isFinite(feedItemId) || feedItemId <= 0) {
    return;
  }

  const previous = stockChangeMap.get(feedItemId);
  const quantityDelta = Number(row?.quantity_delta || 0);
  const currentStock = Number(row?.current_stock);

  stockChangeMap.set(feedItemId, {
    feed_item_id: feedItemId,
    feed_item_name: row?.feed_item_name || previous?.feed_item_name || null,
    quantity_delta:
      Number(previous?.quantity_delta || 0) + (Number.isFinite(quantityDelta) ? quantityDelta : 0),
    unit: row?.unit || previous?.unit || null,
    current_stock: Number.isFinite(currentStock) ? currentStock : previous?.current_stock ?? null,
  });
}

function buildFeedMutationWarnings(stockChanges) {
  const negativeStockByFeedItemId = new Map();

  (Array.isArray(stockChanges) ? stockChanges : []).forEach((row) => {
    const feedItemId = Number(row?.feed_item_id);
    const currentStock = Number(row?.current_stock);
    if (!Number.isFinite(feedItemId) || feedItemId <= 0) {
      return;
  ...
  return {
    negative_stock_items: Array.from(negativeStockByFeedItemId.values()).sort((left, right) =>
      String(left.feed_item_name || '').localeCompare(String(right.feed_item_name || ''))
    ),
  };
}

async function rebuildHorseCalendarPlanEvents(client, horseId, telegramUserId = 'admin_panel') {
  ...
  if (!Number.isFinite(normalizedHorseId) || normalizedHorseId <= 0) {
    return {
      ...
      warnings: buildFeedMutationWarnings([]),
    };
  }

  const today = todayDateString();

  const slotEntriesResult = await client.query(
    `
  ...
  const existingFeedEventsResult = await client.query(
    ...
    WHERE f.horse_id = $1
      AND f.calendar_slot_entry_id IS NOT NULL
      AND f.event_date >= $2
    ORDER BY f.id ASC
    FOR UPDATE OF f, fi
    `,
    [normalizedHorseId, today]
  );
  ...
  for (const row of planResult.rows) {
    ...
    if (
      ...
      String(row.stock_unit || '').trim().toLowerCase() !==
  ...
  const planRowsBySlot = new Map();
  planResult.rows.forEach((row) => {
    const normalizedSlot = normalizeFeedSlot(row.feed_slot);
    if (!normalizedSlot) {
      return;
    }

    const currentRows = planRowsBySlot.get(normalizedSlot) || [];
    currentRows.push(row);
    planRowsBySlot.set(normalizedSlot, currentRows);
  });

  const removableEntryIds = [];
  const activeSlotEntries = [];
  slotEntriesResult.rows.forEach((entry) => {
    const normalizedEventDate = toIsoDateString(entry.event_date);
    if (!normalizedEventDate || normalizedEventDate < today) {
      return;
    }

    const normalizedSlot = normalizeFeedSlot(entry.feed_slot);
    const slotRows = planRowsBySlot.get(normalizedSlot) || [];
    if (slotRows.length === 0) {
      removableEntryIds.push(Number(entry.id));
      return;
    }

    activeSlotEntries.push({
      ...entry,
      event_date: normalizedEventDate,
    });
  });

  const stockChangesByFeedItemId = new Map();
  const restoreByFeedItemId = new Map();
  existingFeedEventsResult.rows.forEach((row) => {
    if (!row.stock_deducted) {
      return;
    }

    accumulateFeedItemQuantity(restoreByFeedItemId, row.feed_item_id, row.quantity);
  });

  for (const [feedItemId, restoreQuantity] of restoreByFeedItemId.entries()) {
    ...
    if (stockResult.rows[0]) {
      recordStockChange(stockChangesByFeedItemId, {
  ...
  if (existingFeedEventsResult.rows.length > 0) {
    await client.query(
      `
      DELETE FROM feed_events
      WHERE horse_id = $1
        AND calendar_slot_entry_id IS NOT NULL
        AND event_date >= $2
      `,
      [normalizedHorseId, today]
    );
  ...
  if (removableEntryIds.length > 0) {
    await client.query(
      `
      DELETE FROM horse_feed_slot_entries
  ...
  activeSlotEntries.forEach((entry) => {
    const slotRows = planRowsBySlot.get(normalizeFeedSlot(entry.feed_slot)) || [];
    slotRows.forEach((row) => {
      if (!row.auto_deduct_stock) {
        return;
      }

      accumulateFeedItemQuantity(requiredStockByFeedItemId, row.feed_item_id, row.quantity);
    });
  });

  const recreatedEvents = [];
  for (const entry of activeSlotEntries) {
    const normalizedSlot = normalizeFeedSlot(entry.feed_slot);
    const slotRows = planRowsBySlot.get(normalizedSlot) || [];
  ...
  for (const [feedItemId, deduction] of requiredStockByFeedItemId.entries()) {
    const stockResult = await client.query(
      `
      UPDATE feed_items
      SET current_stock = current_stock - $1
  ...
  return {
    feed_events: recreatedEvents,
    stock_changes: Array.from(stockChangesByFeedItemId.values()),
    entry_count: activeSlotEntries.length,
    removed_entry_count: removableEntryIds.length,
    warnings: buildFeedMutationWarnings(Array.from(stockChangesByFeedItemId.values())),
  };
}
...
async function ensureFeedPlanningTables() {
  ...
  ensurePromise = (async () => {
    ...
    await ensureTableColumns(pool, 'feed_items', [
      { name: 'unit_cost', definition: 'NUMERIC(12,4)' },
      { name: 'minimum_stock', definition: 'NUMERIC(10,2)' },
      { name: 'category', definition: 'TEXT' },
      { name: 'supplier', definition: 'TEXT' },
      { name: 'purchase_unit_label', definition: 'TEXT' },
      { name: 'purchase_unit_size', definition: 'NUMERIC(10,3)' },
...
async function saveHorseFeedPlanItems({ horseId, items }) {
  ...
  try {
    ...
    for (const item of normalizedItems) {
      ...
      if (!feedItem) {
        throw buildServiceError(`Feed item not found: ${item.feed_item_name}`, 409);
      }
    ...
    const rebuildResult = await rebuildHorseCalendarPlanEvents(client, normalizedHorseId);
    ...
    return {
      ...
      calendar_rebuild: rebuildResult,
      stock_changes: rebuildResult.stock_changes,
      feed_events: rebuildResult.feed_events,
...
async function setHorseFeedSlotChecked({
  ...
  try {
    ...
    if (targetChecked) {
      ...
      const warnings = buildFeedMutationWarnings(stockChanges);
